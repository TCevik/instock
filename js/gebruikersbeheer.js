...
function renderDepartmentBadges(deptVal, maxVisible = 2, fallback = "-") {
  ...
  const badges = visible
    .map((d) => `<span class="department-badge">${escapeHtml(d)}</span>`)
    .join("");
  const more =
    hiddenCount > 0
      ? `<span class="department-badge department-badge-more" title="${escapeHtml(depts.join(", "))}">+${hiddenCount}</span>`
      : "";
...
function getDistinctDepartments(extraDepts = []) {
  ...
  const extraArr = Array.isArray(extraDepts)
    ? extraDepts
    : extraDepts
      ? [extraDepts]
      : [];
...
async function promptNewDepartment(selectInstance) {
  ...
  const nextSelected = Array.isArray(currentSelected)
    ? currentSelected.includes(cleanDept)
      ? currentSelected
      : [...currentSelected, cleanDept]
    : [cleanDept];
...
async function openEditModal(userId) {
  ...
  if (!user) {
    const { data } = await supabase
      .from("user_data")
      .select(
        "user_id, full_name, username, role, departments, birthday, productivity, last_sign_in_at",
      )
      .eq("user_id", userId)
      .maybeSingle();
    user = data;
...
async function loadUsers() {
  ...
  let query = supabase
    .from("user_data")
    .select(
      "user_id, full_name, username, role, departments, birthday, last_sign_in_at",
      { count: "exact" },
    );
  ...
  if (currentUsers.length === 0) {
    ...
    tbody.innerHTML = currentUsers
      .map((user) => {
        ...
        const departmentsHtml = renderDepartmentBadges(
          user.departments,
          2,
          "-",
        );
    ...
    if (cardsContainer) {
      cardsContainer.innerHTML = currentUsers
        .map((user) => {
          const parsed = parseUserDisplay(user.full_name, user.username);
          const displayName = escapeHtml(
            parsed.title || parsed.sub || "Gebruiker",
          );
          const username = escapeHtml(
            parsed.title && parsed.sub ? parsed.sub : "",
          );
          const role = escapeHtml(getRoleLabel(user.role));
          const departmentsHtml = renderDepartmentBadges(
            user.departments,
