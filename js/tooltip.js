...
function formatTooltipText(text) {
  ...
  escaped = escaped.replace(
    /(@[\w.-]+)/g,
    '<span class="tooltip-accent">$1</span>',
  );
  escaped = escaped.replace(/\n/g, "<br>");
  return escaped;
}
...
export function showCustomTooltip(e, data) {
  ...
  if (data.duration && !data.isFlexible) {
    const compareDuration =
      data.totalDuration !== undefined ? data.totalDuration : data.duration;
    const isModified =
      data.origDuration && data.origDuration !== compareDuration;
    if (isModified) {
      durationHtml = `
