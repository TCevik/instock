...
async function initProductivityPage() {
  ...
  try {
    ...
    if (
      errStr.includes("niet ingelogd") ||
      errStr.includes("sessie verlopen") ||
      errStr.includes("unauthorized") ||
      errStr.includes("jwt") ||
      errStr.includes("401")
    ) {
    ...
    showToast(
      "error",
      err.message || "Kon productiviteitsgegevens niet ophalen",
    );
  }
}
...
async function loadTopFillers(userId, date = selectedScoreboardDate) {
  ...
  try {
    ...
    if (topFillers.length === 0) {
      ...
      const ownRank = ownFiller
        ? topFillers.indexOf(ownFiller) + 1
        : data.currentUserRanking
          ? data.currentUserRanking.rank
          : null;
      ...
      if (userPreviewEl) {
        ...
        if (ownData && ownRank) {
          userPreviewEl.appendChild(
            createTopFillerCard(ownData, ownRank, true, isManager),
          );
        }
      }
    ...
    if (
      ...
      sectionEl._userToggledCollapsed === undefined
    ) {
    ...
    if (headerRow && !headerRow._hasToggleListener) {
      ...
      headerRow.addEventListener("click", (e) => {
        if (window.innerWidth <= 768) {
          if (
            e.target.closest("#topFillersDateFilterWrapper") ||
            e.target.closest("#exportTopFillersBtn")
          )
            return;
    ...
    if (sectionDivider) {
      sectionDivider.style.display =
        myShiftsSection && myShiftsSection.style.display !== "none"
          ? "block"
          : "none";
    }
    return data;
...
function createTopFillerCard(filler, rank, isCurrentUser, canClick) {
  ...
  if (canClick) {
    card.setAttribute(
      ...
      isViewed
        ? isCurrentUser
          ? "Je bekijkt nu je eigen productiviteit"
          : "Klik om terug te gaan naar je eigen productiviteit"
        : `Klik om de productiviteit van ${name} te bekijken`,
    );
  }
...
function renderTopFillers(topFillers, container, userId, canClick) {
  ...
  topFillers.forEach((filler, index) => {
    ...
    container.appendChild(
      createTopFillerCard(filler, index + 1, isCurrentUser, canClick),
    );
  });
}
...
function showProductivityUserSkeleton(user) {
  ...
  if (chartSubEl)
    chartSubEl.textContent = `Productiviteit per gewerkte shift van ${name}`;
  if (shiftsHeadingEl)
    shiftsHeadingEl.textContent = `Gewerkte Diensten van ${name}`;
  if (shiftsSubEl)
    shiftsSubEl.textContent = `Overzicht van opgeslagen shifts en behaalde productiviteit van ${name}`;
  ...
  if (colliEl)
    colliEl.innerHTML =
      '<span class="skeleton" style="display:inline-block; width:36px; height:18px; border-radius:4px;"></span>';
  if (daysEl)
    daysEl.innerHTML =
      '<span class="skeleton" style="display:inline-block; width:24px; height:18px; border-radius:4px;"></span>';
...
function applyUserProductivity(user, isSelf) {
  ...
  if (shiftsHeadingEl) {
    shiftsHeadingEl.textContent = isSelf
      ? "Mijn Gewerkte Diensten"
      : `Gewerkte Diensten van ${name}`;
  }
  if (shiftsSubEl) {
...
function updateTopFillerCardSelection() {
  ...
  cards.forEach((card) => {
    ...
    if (card.classList.contains("is-clickable")) {
      const rawName =
        card.querySelector(".top-filler-name")?.textContent || "medewerker";
      ...
      if (isViewed) {
        ...
        card.setAttribute(
          "data-tooltip",
          isSelf
            ? "Klik om je eigen productiviteit te bekijken"
            : `Klik om de productiviteit van ${rawName} te bekijken`,
        );
      }
    }
  });
...
function extractProductivities(prodData) {
  ...
  const addEntry = (item) => {
    ...
    const key = item.date
      ...
      : `${item.finalized_at || ""}_${item.productivity !== undefined ? item.productivity : ""}`;
  ...
  return entries.sort((a, b) => {
    const timeA = (
      parseDate(a.date || a.finalized_at) || new Date(0)
    ).getTime();
    const timeB = (
      parseDate(b.date || b.finalized_at) || new Date(0)
    ).getTime();
    return timeB - timeA;
  });
}
...
function updateChartModeControls() {
  ...
  if (btnIndividual) {
    ...
    btnIndividual.title = isAllowed
      ? ""
      : "Alleen beschikbaar bij 7 shifts, 14 shifts of 1 maand";
...
function renderProductivityChart(entries) {
  ...
  const enriched = chronological.map((e, idx) => {
    ...
    const windowAvg =
      windowProdList.length > 0
        ? Math.round(
            windowProdList.reduce((sum, val) => sum + val, 0) /
              windowProdList.length,
          )
        : rawP;
    ...
    if (!isNaN(dObj.getTime())) {
      dateLabel = dObj.toLocaleDateString("nl-NL", {
        day: "numeric",
        month: "short",
      });
    }
  ...
  if (subtitleEl) {
    const userSuffix =
      selectedFillerUserData && selectedFillerUserId !== currentUserId
  ...
  if (isMonthView) {
    ...
    const monthGroups = Array.from(monthMap.values()).map((g) => {
      const avgRaw = Math.round(
        g.items.reduce((sum, c) => sum + c.rawPercent, 0) / g.items.length,
      );
      const avgRoll = Math.round(
        g.items.reduce((sum, c) => sum + c.avgPercent, 0) / g.items.length,
      );
    ...
    if (monthGroups.length <= maxPoints) {
      ...
      for (let b = 0; b < maxPoints; b++) {
        ...
        const start = Math.max(
          0,
          Math.floor(b * (monthGroups.length / maxPoints)),
        );
        const end = Math.min(
          monthGroups.length,
          Math.floor((b + 1) * (monthGroups.length / maxPoints)),
        );
        const chunk = monthGroups.slice(start, end);
        const items = chunk.length > 0 ? chunk : [monthGroups[centerIdx]];
        const avgRaw = Math.round(
          items.reduce((sum, c) => sum + c.rawPercent, 0) / items.length,
        );
        const avgRoll = Math.round(
          items.reduce((sum, c) => sum + c.avgPercent, 0) / items.length,
        );
    ...
    if (filtered.length <= maxPoints) {
      dataPoints = filtered.map((e) => {
        const percent =
          currentChartMode === "average" ? e.avgPercent : e.rawPercent;
        return {
          ...e,
      ...
      for (let b = 0; b < maxPoints; b++) {
        ...
        const avgRaw = Math.round(
          chunk.reduce((sum, c) => sum + c.rawPercent, 0) / chunk.length,
        );
        const avgRoll = Math.round(
          chunk.reduce((sum, c) => sum + c.avgPercent, 0) / chunk.length,
        );
  ...
  const getY = (val) =>
    padTop + chartH - ((val - minVal) / (maxVal - minVal || 1)) * chartH;
  const getX = (index) => {
  ...
  const dateStep =
    isMobile && coords.length > 5 ? Math.ceil(coords.length / 5) : 1;
    ...
    const taskCards = tasks
      .map((task, tIndex) => {
        const title = escapeHtml(
          task.title || task.pathName || task.name || "Taak",
        );
...
function renderProductivityList(entries, container) {
  ...
  entries.forEach((entry, index) => {
    ...
    const isMostRecent =
      index === 0 && currentShiftPage === 1 && !selectedDateFilter;
    if (!isMostRecent) {
  ...
  if (!exportTopFillersBtnInitialized) {
    ...
    exportBtn.addEventListener("click", (e) => {
      ...
      if (!cachedTopFillers || cachedTopFillers.length === 0) {
        showToast(
          "error",
          "Er zijn geen productiviteitsgegevens om te exporteren.",
        );
        return;
      }

      const dateLabel = selectedScoreboardDate
        ? formatDate(selectedScoreboardDate)
        : "Laatste 10 shifts per vuller";
      try {
        ...
        showToast(
          "error",
          err.message || "Fout bij het exporteren van de top 10",
        );
      }
    });
  }
  ...
  if (prevBtn) {
    prevBtn.addEventListener("click", () => {
      if (currentShiftPage > 1) {
        ...
        renderPaginatedProductivityList();
        document
          .getElementById("myShiftsSection")
...
async function openShiftModal(entry = {}, isEdit = true) {
  ...
  function syncInputsToModalTasks() {
    ...
    items.forEach((itemEl, idx) => {
      ...
      if (inst) {
        ...
        if (inst.startPicker)
          modalTasks[idx].start_time = inst.startPicker.getValue();
        if (inst.endPicker)
          modalTasks[idx].end_time = inst.endPicker.getValue();
  ...
  function renderModalTasks() {
    ...
    tasksListEl.innerHTML = modalTasks
      .map((t, idx) => {
    ...
    modalTasks.forEach((t, idx) => {
      const itemEl = tasksListEl.querySelector(
        `.modal-task-item[data-idx="${idx}"]`,
      );
      if (!itemEl) return;
      ...
      const onTimeChange = () => {
        ...
        if (sVal && eVal) {
          ...
          dur =
            Number(modalTasks[idx].duration_minutes) ||
            Number(modalTasks[idx].duration) ||
            0;
        }
  ...
  if (form) {
    form.addEventListener("submit", async (e) => {
      ...
      const chosenDate = shiftDatePicker
        ? shiftDatePicker.getValue()
        : entry.date || "";
      const newProd =
        Number(overlay.querySelector("#editShiftProd")?.value) || 0;
      const newStart = shiftStartPicker ? shiftStartPicker.getValue() : "";
      const newEnd = shiftEndPicker ? shiftEndPicker.getValue() : "";
      const newPause =
        Number(overlay.querySelector("#editShiftPause")?.value) || 0;
      ...
      try {
        ...
        showToast(
          "notification",
          isEdit
            ? "Dienst en taken succesvol bijgewerkt"
            : "Dienst succesvol toegevoegd",
        );
        ...
        showToast(
          "error",
          err.message ||
            (isEdit ? "Kon dienst niet opslaan" : "Kon dienst niet toevoegen"),
        );
        if (saveBtn) {
          saveBtn.disabled = false;
          saveBtn.textContent = isEdit ? "Opslaan" : "Toevoegen";
