...
import {
  ...
  renderTimelineAxis,
} from "./vulplanning/timeline-axis.js";
...
import {
  ...
  moveAssignedTask,
  unassignAllTasks,
} from "./vulplanning/task-actions.js";
...
} from "./vulplanning/planning-loader.js";
import {
  openComboSettingsModal,
  loadComboSettings,
} from "./vulplanning/combo-settings-modal.js";
...
async function switchToTimelineView() {
  ...
  for (const f of fillers) {
    ...
    if (startMins >= endMins) {
      showToast(
        "error",
        `De begintijd van ${displayName} moet vroeger zijn dan de eindtijd.`,
      );
      return;
    }
    ...
    if (pauseMins > shiftDuration) {
      showToast(
        "error",
        `De pauzetijd van ${displayName} mag niet langer zijn dan de totale werktijd.`,
      );
      return;
    }
    ...
    if (f.username) {
      ...
      if (seenUsernames.has(u)) {
        showToast(
          "error",
          `Medewerker met gebruikersnaam @${f.username} is dubbel ingevoerd.`,
        );
        return;
      }
      seenUsernames.add(u);
  ...
  const hasAssignments = Object.values(planningState.assignedTasks || {}).some(
  ...
  if (hasAssignments) {
    const confirmed = await showConfirmModal({
      ...
      message:
        "Weet je het zeker? De huidige planning wordt hierbij verwijderd.",
  ...
  const existingOtherTasks = (planningState.unassignedTasks || []).filter(
    (t) => t.type === "overige",
  );
...
if (btnClearInput) {
  btnClearInput.addEventListener("click", async () => {
    ...
    const hasAssignments = Object.values(
      planningState.assignedTasks || {},
    ).some((list) => Array.isArray(list) && list.length > 0);
    const hasNames = Array.from(document.querySelectorAll(".vuller-name")).some(
      (inp) => inp.value.trim(),
    );
...
if (clearPlanningBtn) {
  clearPlanningBtn.addEventListener("click", async () => {
    ...
    const hasProdInputs = (planningState.fillers || []).some(
      (f) => f.actualEndTime && f.actualEndTime.trim().length > 0,
    );
    if (!hasAssignments && !hasProdInputs) {
      showToast(
        "notification",
        "Er is geen planning of productiviteit om leeg te maken",
      );
      return;
    }
    ...
    const confirmed = await showConfirmModal({
      ...
      message:
        "Weet je zeker dat je alle toegewezen taken en ingevoerde eindtijden wilt wissen? De taken worden teruggezet naar Onverdeelde Taken.",
      confirmText: "Maak leeg",
      cancelText: "Annuleren",
      isDanger: true,
    });
...
const timelineBoardContainer = document.querySelector(
...
function applyZoom(nextZoom) {
  ...
  if (timelineBoardBody && oldZoom > 0) {
    ...
    const visibleWidth = Math.max(
      0,
      timelineBoardBody.clientWidth - workersWidth,
    );
    const centerTimePx =
      timelineBoardBody.scrollLeft +
      (visibleWidth > 0 ? visibleWidth / 2 : timelineBoardBody.clientWidth / 2);
    ...
    targetScrollLeft = Math.max(
      ...
      centerTimePx * ratio -
        (visibleWidth > 0
          ? visibleWidth / 2
          : timelineBoardBody.clientWidth / 2),
    );
  }
...
if (timelineBoardBody) {
  ...
  timelineBoardBody.addEventListener(
    "scroll",
    () => {
      if (scrollSaveTimeout) clearTimeout(scrollSaveTimeout);
      scrollSaveTimeout = setTimeout(() => {
        localStorage.setItem(
          TIMELINE_SCROLL_LEFT_KEY,
          String(timelineBoardBody.scrollLeft),
        );
        localStorage.setItem(
          TIMELINE_SCROLL_TOP_KEY,
          String(timelineBoardBody.scrollTop),
        );
      }, 150);
    },
    { passive: true },
...
const inputScrollContainer = document.querySelector(
  ".vulplanning-input-scroll-container",
);
if (inputScrollContainer) {
  ...
  inputScrollContainer.addEventListener(
    "scroll",
    () => {
      if (inputScrollTimeout) clearTimeout(inputScrollTimeout);
      inputScrollTimeout = setTimeout(() => {
        localStorage.setItem(
          INPUT_SCROLL_TOP_KEY,
          String(inputScrollContainer.scrollTop),
        );
      }, 150);
    },
    { passive: true },
...
if (btnComboSettings) {
  btnComboSettings.addEventListener("click", () =>
    openComboSettingsModal({
      onRenderUnassigned: doRenderUnassigned,
    }),
  );
