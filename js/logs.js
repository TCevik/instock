...
function getLogFieldIcon(key) {
  ...
  if (k.includes("naam") || k.includes("user") || k.includes("gebruiker"))
    return "person";
  if (k.includes("rol")) return "admin_panel_settings";
  if (k.includes("geboorte") || k.includes("datum") || k.includes("date"))
...
function formatLogValue(key, val) {
  ...
  if (k.includes("prijs")) {
    const num = parseFloat(
      String(clean).replace("€", "").replace(",", ".").trim(),
    );
    if (!isNaN(num)) {
...
function renderPathsDiff(oldPaths, newPaths) {
  ...
  newList.forEach((newP, newIdx) => {
    ...
    if (newP.id) {
      const oldIdx = oldList.findIndex(
        (oldP, idx) =>
          !matchedOldIndices.has(idx) && oldP && oldP.id && oldP.id === newP.id,
      );
      if (oldIdx !== -1) {
  ...
  newList.forEach((newP, newIdx) => {
    ...
    const oldIdx = oldList.findIndex(
      (oldP, idx) =>
        !matchedOldIndices.has(idx) && oldP && norm(oldP.name) === targetName,
    );
    if (oldIdx !== -1) {
      matchedOldIndices.add(oldIdx);
      matchedNewIndices.add(newIdx);
  ...
  pairs.forEach(({ oldP, newP }) => {
    ...
    newCats.forEach((newC, nIdx) => {
      if (!newC) return;
      if (newC.id) {
        const oIdx = oldCats.findIndex(
          (oldC, idx) =>
            !matchedOldCatIndices.has(idx) &&
            oldC &&
            oldC.id &&
            oldC.id === newC.id,
        );
    ...
    newCats.forEach((newC, nIdx) => {
      ...
      const oIdx = oldCats.findIndex(
        (oldC, idx) =>
          !matchedOldCatIndices.has(idx) && oldC && norm(oldC.name) === cName,
      );
      if (oIdx !== -1) {
        matchedOldCatIndices.add(oIdx);
        matchedNewCatIndices.add(nIdx);
    ...
    const addedCats = [];
    newCats.forEach((newC, nIdx) => {
      if (!matchedNewCatIndices.has(nIdx) && newC && newC.name)
        addedCats.push(newC);
    });

    const removedCats = [];
    oldCats.forEach((oldC, oIdx) => {
      if (!matchedOldCatIndices.has(oIdx) && oldC && oldC.name)
        removedCats.push(oldC);
    });
...
function renderBakplanDiff(oldVal, newVal) {
  ...
  function renderCategoryTable(oldC, newC) {
    const oldItems = Array.isArray(oldC?.items || oldC?.artikelen)
      ? oldC.items || oldC.artikelen
      : [];
    const newItems = Array.isArray(newC?.items || newC?.artikelen)
      ? newC.items || newC.artikelen
      : [];
    ...
    newItems.forEach((nItem, nIdx) => {
      ...
      const oIdx = oldItems.findIndex(
        (oItem, idx) =>
          !matchedOldItemIdx.has(idx) &&
          oItem &&
          ((nItem.id && oItem.id === nItem.id) ||
            (nOmsch && norm(oItem.omschrijving) === nOmsch)),
      );
      if (oIdx !== -1) {
    ...
    const tableRowsHtml = rows
      .map((r) => {
        ...
        const perPlaatCell = renderCell(
          oItem.perPlaat ?? oItem.per_plaat ?? null,
          nItem.perPlaat ?? nItem.per_plaat ?? null,
          false,
          r.type,
        );
        const prijsCell = renderCell(
          oItem.prijs ?? null,
          nItem.prijs ?? null,
          true,
          r.type,
        );
        const promoCell = renderCell(
          oItem.promo ?? null,
          nItem.promo ?? null,
          true,
          r.type,
        );
        const opleggenCell = renderCell(
          oItem.opleggen ?? null,
          nItem.opleggen ?? null,
          false,
          r.type,
        );
        const dervingCell = renderCell(
          oItem.derving ?? null,
          nItem.derving ?? null,
          false,
          r.type,
  ...
  newList.forEach((newC, nIdx) => {
    ...
    const cId = newC.id;
    if (cId) {
      const oIdx = oldList.findIndex(
        (oldC, idx) => !matchedOldIndices.has(idx) && oldC && oldC.id === cId,
      );
      if (oIdx !== -1) {
  ...
  newList.forEach((newC, nIdx) => {
    ...
    const oIdx = oldList.findIndex(
      (oldC, idx) =>
        ...
        oldC &&
        norm(oldC.name || oldC.categorie) === cName,
    );
    if (oIdx !== -1) {
      matchedOldIndices.add(oIdx);
      matchedNewIndices.add(nIdx);
  ...
  let cardsCount = 0;
  const cardsHtml = catChanges
    .map((change) => {
      if (change.type === "added") {
        cardsCount++;
        const catName = escapeHtml(
          change.newC.name || change.newC.categorie || "Nieuwe categorie",
        );
      ...
      if (change.type === "removed") {
        cardsCount++;
        const catName = escapeHtml(
          change.oldC.name || change.oldC.categorie || "Verwijderde categorie",
        );
  ...
  return {
    ...
    html:
      cardsHtml ||
      '<div class="empty-state" style="padding: 16px 0;">Geen inhoudelijke wijzigingen in het bakplan gedetecteerd.</div>',
  };
}
...
function renderShiftDiff(oldShift, newShift) {
  ...
  const oldProd =
    oldObj.productivity !== undefined
      ? Math.round(Number(oldObj.productivity))
      : null;
  const newProd =
    newObj.productivity !== undefined
      ? Math.round(Number(newObj.productivity))
      : null;
  ...
  for (let i = 0; i < maxLen; i++) {
    ...
    if (!oT && nT) {
      ...
      if (nT.start_time || nT.end_time)
        details.push(`${nT.start_time || ""} - ${nT.end_time || ""}`);
      ...
      const tFieldDiffs = [];
      if (oTitle !== nTitle)
        tFieldDiffs.push({ label: "Naam", oldVal: oTitle, newVal: nTitle });
      if (oType !== nType)
        tFieldDiffs.push({ label: "Type", oldVal: oType, newVal: nType });
      if (oColli !== nColli)
        tFieldDiffs.push({
          label: "Colli",
          oldVal: `${oColli} colli`,
...
function getAffectedDisplay(affected, action = "", log = null) {
  ...
  if (String(affected).toLowerCase() === "productiviteit") {
    ...
    if (log?.new_value?.username) {
      const parsed = parseUserDisplay(
        log.new_value.full_name,
        log.new_value.username,
      );
      return {
        title: parsed.title || "Medewerker",
        sub: parsed.sub,
  ...
  const isUser =
    action.toLowerCase().includes("gebruiker") ||
    action.toLowerCase().includes("dienst") ||
    action.toLowerCase().includes("taak") ||
    String(affected).includes("@");
...
function renderTable() {
  ...
  if (currentLogs.length === 0) {
    ...
    tbody.innerHTML = currentLogs
      .map((log, idx) => {
        ...
        const displayName =
          parsedUser.title || (log.user_id ? "Onbekende gebruiker" : "Systeem");
        const subName = parsedUser.sub;

        const action = getLogAction(log);
        let affectedHtml = "-";
        const aff = getAffectedDisplay(
          log.affected ?? log.affected_user,
          action,
          log,
        );
        if (aff) {
  ...
  if (cardsContainer) {
    if (currentLogs.length === 0) {
      ...
      cardsContainer.innerHTML = currentLogs
        .map((log, idx) => {
          const user = usersMap.get(log.user_id);
          const parsedUser = parseUserDisplay(user?.full_name, user?.username);
          const displayName =
            parsedUser.title ||
            (log.user_id ? "Onbekende gebruiker" : "Systeem");
          const action = getLogAction(log);
          const isDanger = action.toLowerCase().includes("verwijderd");
          const timeStr = formatDateTime(log.happened_at || log.created_at);
          const aff = getAffectedDisplay(
...
function openDetailsModal(log) {
  ...
  if (newVal && typeof newVal === "object") {
    if (
      newVal.type === "finalize_productivity" &&
      ...
      isProductivityLog = true;
      const shiftObj =
        newVal.shift || (newVal.type === "delete_shift" ? oldVal : null);
      const targetUser =
        (newVal.user_id && usersMap.get(newVal.user_id)) || null;
        ...
        prodItems = [
          {
            ...
            prev_productivity:
              oldVal?.productivity !== undefined ? oldVal.productivity : null,
            date: shiftObj.date || oldVal?.date || "",
            shift: shiftObj.shift || null,
            total_colli: calculateShiftTotalColli(shiftObj),
            task_count: Array.isArray(shiftObj.tasks)
              ? shiftObj.tasks.length
              : 0,
          },
        ];
      ...
      Array.isArray(newVal.items) &&
      (String(action).toLowerCase().includes("productiviteit") ||
        String(log.affected).toLowerCase() === "productiviteit")
    ) {
      isProductivityLog = true;
      prodItems = newVal.items;
      prodBatchDate = newVal.date || "";
      ...
      prodItems = [
        {
          ...
          prev_productivity: oldVal?.productivity
            ? typeof oldVal.productivity === "object"
              ? oldVal.productivity.productivity
              : oldVal.productivity
            : null,
          date: typeof p === "object" ? p.date : "",
          shift: typeof p === "object" ? p.shift : null,
          total_colli:
            typeof p === "object" ? calculateShiftTotalColli(p) : null,
          task_count:
            typeof p === "object" && Array.isArray(p.tasks)
              ? p.tasks.length
              : 0,
        },
      ];
    ...
    detailsHtml = `
            <div class="log-prod-wrapper">
                ...
                <div class="log-prod-list">
                    ${prodItems
                      .map((item) => {
                        const parsedItem = parseUserDisplay(
                          item.full_name,
                          item.username,
                        );
                        const name = escapeHtml(
                          parsedItem.title || "Medewerker",
                        );
                        const uname = parsedItem.sub
                          ? escapeHtml(parsedItem.sub)
                          : "";
                        const pct = Number(item.productivity || 0);
                        const statusClass = getProductivityStatusClass(pct);
                        const dateStr = item.date
                          ? formatDutchDate(item.date)
                          : "";
                        ...
                        if (item.shift && item.shift.actual_end) {
                          subParts.push(
                            `Eind: ${escapeHtml(item.shift.actual_end)}`,
                          );
                        } else if (
                          item.shift &&
                          item.shift.start &&
                          item.shift.planned_end
                        ) {
                          subParts.push(
                            `${escapeHtml(item.shift.start)} - ${escapeHtml(item.shift.planned_end)}`,
                          );
                        }
                        if (item.task_count > 0) {
                          subParts.push(
                            `${item.task_count} ${item.task_count === 1 ? "taak" : "taken"}`,
                          );
                        }
                        ...
                        let prevTagHtml = "";
                        if (
                          item.prev_productivity !== null &&
                          item.prev_productivity !== undefined
                        ) {
    ...
    if (allKeys.size === 0) {
      ...
      allKeys.forEach((key) => {
        ...
        if (
          oldField !== null &&
          newField !== null &&
          JSON.stringify(oldField) === JSON.stringify(newField)
        )
          return;
        ...
        if (key === "bakplan" || key === "data") {
          const diffResult = renderBakplanDiff(
            oldField || oldVal,
            newField || newVal,
          );
          if (diffResult.count > 0) {
        ...
        function renderValHtml(formatted, type) {
          ...
          const iconName =
            type === "new" ? "check" : type === "old" ? "close" : "info";
          return `
...
async function initActionFilter() {
  ...
  const uniqueActions = Array.from(
    new Set(data.map((l) => l.action).filter(Boolean)),
  ).sort();
  const actionFilterContainer = document.getElementById(
...
if (thTime) {
  thTime.addEventListener("click", () => {
    ...
    if (sortTimeIcon) {
      sortTimeIcon.textContent = sortAscending
        ? "arrow_upward"
        : "arrow_downward";
    }
    currentPage = 1;
