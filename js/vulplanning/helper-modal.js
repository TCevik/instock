...
export async function openHelperModal(options) {
  ...
  const sourceFiller = planningState.fillers.find(
    (f) => f.id === sourceFillerId,
  );
  ...
  const totalTaskDuration =
    task.duration + existingHelpers.reduce((sum, h) => sum + h.duration, 0);
  ...
  if (helpers.length === 0) {
    const firstCandidate = planningState.fillers.find(
      (f) => f.id !== sourceFillerId,
    );
    if (firstCandidate) {
      helpers.push({
  ...
  function calculateHelperRemaining(fillerId) {
    ...
    const assigned = (planningState.assignedTasks[fillerId] || []).reduce(
      (sum, t) => {
  ...
  function calculateGiverNeeded() {
    ...
    const assignedWithout = (
      planningState.assignedTasks[sourceFillerId] || []
    ).reduce((sum, t) => {
  ...
  function renderHelpersList() {
    ...
    const availableFillers = planningState.fillers.filter(
      (f) => f.id !== sourceFillerId,
    );
    const canAddMore =
      helpers.length < MAX_HELPERS && helpers.length < availableFillers.length;
    ...
    helpers.forEach((helper, idx) => {
      ...
      const selectedIds = new Set(
        helpers.map((h, i) => (i !== idx ? h.fillerId : null)).filter(Boolean),
      );
  ...
  function updateQuickButtons() {
    ...
    if (giverDesc) {
      giverDesc.textContent = canFillGiver
        ? `${giverNeeded}m behouden`
        : `${giverNeeded}m nodig`;
    }
  }

  function updatePreview() {
    const sumHelpers = helpers.reduce(
      (sum, h) => sum + (parseInt(h.duration, 10) || 0),
      0,
    );
        ...
        if (helpers[idx]) {
          ...
          if (remaining > 0 && remaining < totalTaskDuration) {
            ...
            const input = overlay.querySelector(
              `.helper-mins-input[data-index="${idx}"]`,
            );
  ...
  form.addEventListener("submit", (e) => {
    ...
    showToast(
      "notification",
      `${helpers.length} helper(s) toegewezen voor ${baseTitle}`,
    );
  });
