...
export function detectPdfType(text) {
  ...
  if (
    lower.includes("overzicht colli") ||
  ...
  for (let line of lines) {
    if (
      /(\b(?:[01]?[0-9]|2[0-3]):[0-5][0-9])\s*-\s*((?:[01]?[0-9]|2[0-3]):[0-5][0-9]|(?:[01]?[0-9]|2[0-3])(?::(?:\.\.\.|\u2026)?|(?:\.\.\.|\u2026))?(?=\s|[|]|$))/.test(
        line,
      )
    ) {
      hasTimeRange = true;
...
export function openUnifiedImportModal({
  onRoosterImport,
  onColliImport,
  availableUsers = [],
}) {
  ...
  showModal(modalContent).then((overlay) => {
    ...
    async function processFiles(files) {
        ...
        try {
          ...
          if (type === "rooster") {
            ...
            if (
              ...
              typeof onRoosterImport === "function"
            ) {
            ...
            if (shifts.length > 0 && typeof onRoosterImport === "function") {
              ...
              Object.keys(colli).length > 0 &&
              typeof onColliImport === "function"
            ) {
              onColliImport(colli);
              importedColli = true;
              ...
              showToast(
                "error",
                `Kon type van ${file.name} niet automatisch herkennen.`,
              );
            }
          }
        } catch (err) {
          showToast(
            "error",
            `Fout bij verwerken van ${file.name}: ` + err.message,
          );
        }
      }
      ...
      if (importedRooster && importedColli) {
        showToast(
          "notification",
          `Dagrooster (${totalShifts} medewerkers) & Colli overzicht succesvol geïmporteerd!`,
        );
      } else if (importedRooster) {
        showToast(
          "notification",
          `Dagrooster succesvol geïmporteerd (${totalShifts} medewerkers)!`,
        );
