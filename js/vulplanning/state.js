...
export function getDraggedTask() {
  ...
  if (draggedTaskData.source === "unassigned") {
    if (
      draggedTaskData.taskId === "pauze_template" ||
      (typeof draggedTaskData.taskId === "string" &&
        draggedTaskData.taskId.startsWith("pauze"))
    ) {
      return {
        id: "pauze_template",
        type: "pauze",
    ...
    return (
      planningState.unassignedTasks.find(
        (t) => t.id === draggedTaskData.taskId,
      ) || null
    );
  }
  if (
    draggedTaskData.source === "assigned" ||
    draggedTaskData.source === "sidebar_assigned"
  ) {
