...
export async function loadSavedPlanning(options = {}) {
  ...
  try {
    ...
    const savedFillers = (Array.isArray(data.fillers) ? data.fillers : []).map(
      (f, idx) => ({
        ...f,
        customOrder:
          f && typeof f.customOrder === "number" ? f.customOrder : idx,
      }),
    );
    ...
    planningState.unassignedTasks = Array.isArray(scheduleData.unassigned_tasks)
      ...
      : Array.isArray(data.unassigned_tasks)
        ? data.unassigned_tasks
        : [];
    planningState.assignedTasks =
      scheduleData.assigned_tasks &&
      typeof scheduleData.assigned_tasks === "object"
        ? scheduleData.assigned_tasks
        : data.assigned_tasks && typeof data.assigned_tasks === "object"
          ? data.assigned_tasks
          : {};
    ...
    const rawSchedule =
      data.schedule && typeof data.schedule === "object" ? data.schedule : {};
    const scheduleAssignments = rawSchedule.assigned_tasks
      ? rawSchedule.assigned_tasks
      : rawSchedule;
      ...
      if (Array.isArray(taskRefs)) {
        ...
        taskRefs.forEach((ref) => {
          ...
          if (ref && (ref.type === "overige" || ref.type === "pauze")) {
            ...
            const dur =
              (ref && ref.duration) || (template && template.duration) || 30;
            hydratedAssignedTasks[fillerId].push({
              ...
              title:
                ...
                (ref.type === "pauze" ? "Pauze" : "Overige taak"),
              duration: dur,
              origDuration:
                ref && ref.origDuration !== undefined
                  ? ref.origDuration
                  : template
                    ? template.duration
                    : dur,
            ...
            hydratedAssignedTasks[fillerId].push({
              ...
              origDuration:
                (ref && ref.origDuration) || (parent && parent.duration) || 30,
    ...
    const unassignedNormal = generatedTasks.filter(
