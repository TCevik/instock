...
export function sortFillersByNameAsc(a, b) {
    const comp = (a?.name || '').localeCompare(b?.name || '', 'nl', { sensitivity: 'base' });
    if (comp !== 0) return comp;
    return (timeToMinutes(a?.from) || 9999) - (timeToMinutes(b?.from) || 9999);
}

export function sortFillersByNameDesc(a, b) {
    const comp = (b?.name || '').localeCompare(a?.name || '', 'nl', { sensitivity: 'base' });
    if (comp !== 0) return comp;
    return (timeToMinutes(a?.from) || 9999) - (timeToMinutes(b?.from) || 9999);
}
...
export function sortFillers(sortType, callbacks = {}, record = true, syncRooster = true) {
    if (!planningState.fillers || planningState.fillers.length === 0) return;

    ensureCustomOrder();

    if (sortType === 'custom') {
        planningState.fillers.sort((a, b) => (Number(a?.customOrder) || 0) - (Number(b?.customOrder) || 0));
        currentSortType = null;
        try {
            localStorage.removeItem(SORT_STORAGE_KEY);
        } catch (_) {}
...
export function applyStoredFillerSort(callbacks = {}) {
    if (!Array.isArray(planningState.fillers) || planningState.fillers.length === 0) return;
    ensureCustomOrder();
    let savedSort = null;
    try {
    ...
    const validSorts = ['start-asc', 'start-desc', 'end-asc', 'end-desc', 'name-asc', 'name-desc'];
...
export function initFillerSort(callbacks = {}) {
    ...
    if (headerName && !headerName.__sortBound) {
        ...
        headerName.addEventListener('click', () => {
            closeFillerSortMenu();
            const nextSort = currentSortType === 'name-asc' ? 'name-desc' : 'name-asc';
