...
export function parseColliPdfText(rawText) {
    ...
    for (let rawLine of lines) {
        ...
        if (line.includes('Overzicht colli') ||
        ...
        const match = line.match(/^(?:99999\s*)?(\d{1,4})\s+(.+?)\s+(\d+)(?:\s+[A-Z0-9].*)?$/i);
        if (match) {
...
export function arePathsMatchingDefault(currentPaths) {
    ...
    for (let i = 0; i < expectedStructure.length; i++) {
        ...
        if ((curPath.name || '').trim().toLowerCase() !== expPath.name.toLowerCase()) return false;
        ...
        for (let j = 0; j < expPath.categories.length; j++) {
            ...
            if ((curCat.name || '').trim().toLowerCase() !== expCat.name.toLowerCase()) return false;
        }
    }
    ...
    showModal(modalContent).then(overlay => {
        ...
        async function handleFile(file) {
            ...
            try {
                ...
                dropzoneText.textContent = 'Sleep je colli PDF hierheen of klik om te kiezen';
