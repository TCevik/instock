...
export function cleanImportedName(rawName, availableUsers = []) {
  ...
  if (availableUsers && availableUsers.length > 0) {
    ...
    if (exactMatches.length === 1) {
      return (
        exactMatches[0].full_name?.trim() ||
        exactMatches[0].username?.trim() ||
        name
      );
    }
    ...
    const accentExactMatches = availableUsers.filter((u) => {
      const fullName = stripDiacritics(
        (u.full_name || "").toLowerCase().trim(),
      );
      const username = stripDiacritics((u.username || "").toLowerCase().trim());
      return (
        fullName === normQ || username === normQ || `@${username}` === normQ
      );
    });

    if (accentExactMatches.length === 1) {
      return (
        accentExactMatches[0].full_name?.trim() ||
        accentExactMatches[0].username?.trim() ||
        name
      );
    }
    ...
    if (containsMatches.length === 1) {
      return (
        containsMatches[0].full_name?.trim() ||
        containsMatches[0].username?.trim() ||
        name
      );
    }
    ...
    if (accentContainsMatches.length === 1) {
      return (
        accentContainsMatches[0].full_name?.trim() ||
        accentContainsMatches[0].username?.trim() ||
        name
      );
    }
  }
...
export function parseShiftText(rawText, availableUsers = []) {
  ...
  for (let rawLine of lines) {
    ...
    if (
      /^(0[0-9]|1[0-9]|2[0-3]):[0-5][0-9]\s+(0[0-9]|1[0-9]|2[0-3]):[0-5][0-9]/.test(
        line,
      )
    ) {
      continue;
    ...
    const timeMatches = [
      ...line.matchAll(
        /(\b(?:[01]?[0-9]|2[0-3]):[0-5][0-9])\s*-\s*((?:[01]?[0-9]|2[0-3]):[0-5][0-9]|(?:[01]?[0-9]|2[0-3])(?::(?:\.\.\.|\u2026)?|(?:\.\.\.|\u2026))?(?=\s|[|]|$))/g,
      ),
    ];
    ...
    rawName = rawName
      .replace(/^(Goederenverwerking|Verkoop|Kassa|Vakkenvullen)\s+/i, "")
      .trim();
    ...
    let pauseStr = "0 min";
    const pauseMatches = [
      ...line.matchAll(/\b(0[0-9]|1[0-9]|2[0-3]):([0-5][0-9])\b/g),
    ];
    ...
    async function handleFile(file) {
      ...
      try {
        ...
        dropzoneText.textContent =
          "Sleep je dagrooster PDF hierheen of klik om te kiezen";
