...
export async function openWorkerModal(filler = null, callbacks = {}) {
  ...
  const subtitle = isEdit
    ...
    : "Voeg een medewerker toe aan de planning.";
  ...
  if (deleteBtn && isEdit) {
    deleteBtn.addEventListener("click", async () => {
      const hasAssigned =
        (planningState.assignedTasks[filler.id] || []).length > 0;
      ...
      planningState.fillers = planningState.fillers.filter(
        (f) => f.id !== filler.id,
      );
  ...
  form.addEventListener("submit", (e) => {
    ...
    if (pauseMins > shiftDuration) {
      showToast(
        "error",
        "De pauzetijd mag niet langer zijn dan de totale werktijd.",
      );
      return;
    }
    ...
    if (username) {
      ...
      if (isDuplicate) {
        showToast(
          "error",
          `Er staat al een medewerker met gebruikersnaam @${username} in de planning.`,
        );
        return;
      }
    }
    ...
    if (isEdit) {
      ...
      const newId =
        planningState.fillers.reduce(
          (max, f) => (typeof f.id === "number" ? Math.max(max, f.id) : max),
          0,
        ) + 1;
      const newFiller = {
        id: newId,
