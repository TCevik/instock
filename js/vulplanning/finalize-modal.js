...
export async function openFinalizeModal() {
  ...
  if (datePickerContainer) {
    finalizeDatePicker = createDatePicker(
      datePickerContainer,
      todayStr,
      async (selectedDateVal) => {
        if (!selectedDateVal) return;
        if (listContainer) {
          listContainer.innerHTML = renderSkeletonWorkers(
            Math.max(2, fillersWithUser.length || 3),
          );
        }
  ...
  (planningState.fillers || []).forEach((filler) => {
    ...
    const hasEndTime = !!(
      ...
      String(filler.actualEndTime).trim().length >= 4 &&
      stats.prodResult
    );
  ...
  async function recalculateStatusesForDate(selectedDateStr) {
    ...
    fillersWithUser.forEach((item) => {
      ...
      if (existingRecord && hasEndTime) {
        ...
        const oldEnd = String(
          existingRecord.actual_end_time ||
            existingRecord.shift?.actual_end ||
            "",
        ).trim();
        ...
        const oldTasksLen = Array.isArray(existingRecord.tasks)
          ? existingRecord.tasks.length
          : null;
        const newTasksLen = assigned.length;

        const sameProd =
          Number(item.prevProdPercent) === Number(stats.prodResult?.percent);
  ...
  function applySortingAndCounts() {
    ...
    const readyCount = fillersWithUser.filter(
      (f) => f.hasEndTime && !f.isIdentical,
    ).length;
    const overwriteCount = fillersWithUser.filter(
      (f) => f.hasEndTime && f.isOverwrite,
    ).length;
    const identicalCount = fillersWithUser.filter(
      (f) => f.hasEndTime && f.isIdentical,
    ).length;
    ...
    return items
      .map((item, idx) => {
        ...
        const displayName = escapeHtml(
          f.name || item.resolvedUser?.full_name || "Medewerker",
        );
        ...
        if (item.hasEndTime) {
          ...
          const itemClass = item.isIdentical
            ? "is-disabled"
            : item.checked
              ? "is-checked"
              : "";
  ...
  if (fillersWithUser.length === 0) {
    ...
    showToast(
      "error",
      "Geen medewerkers met een gekoppelde gebruikersnaam gevonden in de planning.",
    );
    return;
  }
  ...
  function updateUiState() {
    const checkedCount = fillersWithUser.filter(
      (f) => f.hasEndTime && !f.isIdentical && f.checked,
    ).length;
    const eligibleCount = fillersWithUser.filter(
      (f) => f.hasEndTime && !f.isIdentical,
    ).length;
    ...
    if (btnFinalizeText) {
      btnFinalizeText.textContent =
        checkedCount > 0 ? `Finaliseren (${checkedCount})` : "Finaliseren";
    }
    if (btnSubmit) {
    ...
    if (selectAllCheckbox) {
      ...
      selectAllCheckbox.checked =
        eligibleCount > 0 && checkedCount === eligibleCount;
      selectAllCheckbox.indeterminate =
        checkedCount > 0 && checkedCount < eligibleCount;
    }
  }
  ...
  if (btnSubmit) {
    btnSubmit.addEventListener("click", async () => {
      const selected = fillersWithUser.filter(
        (f) => f.hasEndTime && !f.isIdentical && f.checked,
      );
      if (selected.length === 0) return;

      const targetDate =
        (finalizeDatePicker ? finalizeDatePicker.getValue() : "") || todayStr;

      const now = new Date();
      const currentMins = now.getHours() * 60 + now.getMinutes();
      const isTargetToday = targetDate === todayStr;

      const futureEndItems = isTargetToday
      ...
      if (futureEndItems.length > 0) {
        const names = futureEndItems
          .map((i) => i.filler.name || i.username)
          .join(", ");
      ...
      try {
        const records = selected.map((item) => {
          const formattedTasks = getFormattedTasksWithTimes(
            item.filler,
            item.assigned,
          );
          const prodPercent = item.stats.prodResult
            ? item.stats.prodResult.percent
            : 0;
        ...
        showToast(
          "success",
          `${finalizedCount} ${finalizedCount === 1 ? "medewerker" : "medewerkers"} succesvol gefinaliseerd!`,
        );
        closeModal();
      } catch (err) {
        showToast(
          "error",
          err.message || "Er is een fout opgetreden bij het finaliseren",
        );
