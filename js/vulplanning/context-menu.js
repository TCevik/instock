    ...
    if (taskIndex === assignedList.length - 1) {
      ...
      if (filler) {
        ...
        const targetShiftDuration =
          Math.max(0, shiftEnd - shiftStart - presetPause) + assignedPauzeMins;
...
export async function openEditCustomTaskModal(
  task,
  isAssigned,
  fillerId,
  taskIndex,
  ...
  if (
    mainTask.type === "vullen" &&
    Array.isArray(mainTask.categoryDetails) &&
    mainTask.categoryDetails.length > 0
  ) {
    let totalMins = 0;
    ...
    if (totalMins > 0) {
      ...
      if (
        origDuration === undefined ||
        origDuration === null ||
        (origDuration === mainTask.duration && calculated !== mainTask.duration)
      ) {
  ...
  const hasModifiedDuration =
    isAssigned &&
    origDuration !== undefined &&
    origDuration !== null &&
    origDuration !== mainTask.duration;
  const hasModifiedTitle =
    isAssigned &&
    origTitle !== undefined &&
    origTitle !== null &&
    origTitle !== mainTask.title;
  const canRestore = hasModifiedDuration || hasModifiedTitle;

  const mainFillerObj = isAssigned
    ? planningState.fillers.find((f) => f.id === mainFillerId)
    : null;
  const mainFillerName = mainFillerObj
    ? mainFillerObj.name || "Medewerker"
    : "";

  let totalWorkMinutes =
    mainTask.duration +
    helpers.reduce((sum, h) => sum + (h.task.duration || 0), 0);
  ...
  if (helpers.length > 0) {
    ...
    subtitle =
      "Pas de tijdsduur of titel aan voor deze planning. De originele gegevens blijven behouden.";
  } else {
    subtitle = "Pas de taakomschrijving of standaardtijd aan in het overzicht.";
  }

  const helpersRowsHtml = helpers
    .map((h, idx) => {
      const helperFillerObj = planningState.fillers.find(
        (f) => f.id === h.fillerId,
      );
      const helperName = helperFillerObj
        ? helperFillerObj.name || "Helper"
        : "Helper";
      return `
  ...
  const helperInputs = Array.from(
    overlay.querySelectorAll(".helper-dur-input"),
  );
  const currentPersonSumDisplay = overlay.querySelector(
    "#currentPersonSumDisplay",
  );
  const totalWorkTargetDisplay = overlay.querySelector(
    "#totalWorkTargetDisplay",
  );

  const getAllDurInputs = () => [durInput, ...helperInputs];

  const calculateCurrentSum = () => {
    return getAllDurInputs().reduce(
      (sum, inp) => sum + (parseInt(inp.value, 10) || 0),
      0,
    );
  };
  ...
  const validateAllInputs = () => {
    ...
    if (totalWorkInput) {
      ...
      if (isNaN(totalWorkVal) || totalWorkVal < 1) {
        ...
        durError.textContent =
          "Voer een geldige totale werktijd in (minimaal 1 minuut).";
    ...
    if (totalWorkTargetDisplay)
      totalWorkTargetDisplay.textContent = totalWorkVal;
    if (currentPersonSumDisplay)
      currentPersonSumDisplay.textContent = currentSum;
  ...
  helperInputs.forEach((hInp) =>
    hInp.addEventListener("input", validateAllInputs),
  );
  ...
  form.addEventListener("submit", async (ev) => {
    ...
    if (newTitle && newMainDuration > 0) {
      if (isAssigned) {
        if (
          mainTask.origDuration === undefined ||
          mainTask.origDuration === null
        ) {
        ...
        helpers.forEach((h, idx) => {
          ...
          if (
            h.task.origDuration === undefined ||
            h.task.origDuration === null
          ) {
      ...
      showToast(
        "notification",
        isAssigned
          ? "Taak en helper tijden bijgewerkt"
