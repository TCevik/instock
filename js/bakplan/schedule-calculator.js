...
export function getPlateQuantity(item) {
  const val = parseFloat(item.perPlaat);
  return !isNaN(val) && val > 0 ? val : 12;
}

export function generateBakplanSchedule(
  bakplanCategories,
  dayName = "maandag",
  customCarts = null,
) {
  const cartsConfig =
    customCarts && customCarts.length > 0 ? customCarts : DEFAULT_CARTS;
  ...
  (bakplanCategories || []).forEach((catObj) => {
    ...
    const isThawCat =
      ...
      (catObj.cartType === "ontdooi" ||
        cartsConfig.some(
          (c) =>
            (!c.oven || c.type === "thaw") && c.reservedCategory === catName,
        ));

    (catObj.items || []).forEach((prod) => {
      if (!prod.omschrijving || prod.omschrijving.trim() === "") return;
      const dayVals =
        prod.days && prod.days[dayName] ? prod.days[dayName] : prod;
      const opleggenNum = parseFloat(dayVals.opleggen);

      if (isNaN(opleggenNum) || opleggenNum <= 0) return;

      const plateQty = getPlateQuantity(prod);
      const exactPlaten = opleggenNum / plateQty;
      if (exactPlaten <= 0) return;

      const item = {
        category: catName,
        description: prod.omschrijving,
  ...
  const createPhysicalPlatesForCat = (prodItems) => {
    ...
    wholePlates.forEach((wp) => {
      physicalPlates.push({
        products: [
          { description: wp.description, val: wp.val, pieces: wp.pieces },
        ],
        platen: 1,
      });
    });
          ...
          products: [
            {
              ...
              pieces: Math.round(current.fraction * current.plateQty),
            },
            {
              description: partner.description,
              val: partner.fraction,
              pieces: Math.round(partner.fraction * partner.plateQty),
            },
          ],
          platen: 1,
        });
  ...
  const bakeCatNames = Array.from(
    new Set(allBakeProducts.map((p) => p.category)),
  );
  ...
  bakeCatNames.forEach((catName) => {
    ...
    const uniqueDescsInCat = Array.from(
      new Set(plates.flatMap((p) => p.products.map((pr) => pr.description))),
    );
    ...
    uniqueDescsInCat.forEach((desc) => {
      const idx = plates.findIndex(
        (p, i) =>
          ...
          p.products.some((pr) => pr.description === desc),
      );
      if (idx > -1) {
  ...
  const batch1OvenCapacity = cartsConfig
    .filter((c) => c.oven)
  ...
  restPool.forEach((item) => {
    const desc =
      item.plate && item.plate.products && item.plate.products[0]
        ? item.plate.products[0].description
        : item.category;
  ...
  const unrollUniqueFirst = (pool) => {
    ...
    pool.forEach((item) => {
      const descKey =
        item.plate && item.plate.products
          ? item.plate.products
              .map((p) => p.description)
              .sort()
              .join("|")
  ...
  const fillCartsForBatchPool = (batchNumber, poolItems, thawProds) => {
    ...
    if (batchNumber === 1 && thawProds.length > 0) {
      ...
      if (thawCarts.length > 0) {
        const thawCatNames = Array.from(
          new Set(thawProds.map((p) => p.category)),
        );
        const thawPlates = [];
    ...
    const remainingPool = [...poolItems].sort((a, b) => {
      ...
      const descA =
        a.plate && a.plate.products && a.plate.products[0]
          ? a.plate.products[0].description.toLowerCase()
          : "";
      const descB =
        b.plate && b.plate.products && b.plate.products[0]
          ? b.plate.products[0].description.toLowerCase()
          : "";
    ...
    singleCarts.forEach((cart) => {
      ...
      if (cart.reservedCategory) {
        ...
        selectedCat =
          sortedCats.find((c) => catCounts[c] >= cart.capacity) ||
          sortedCats[0];
      }
    ...
    mixedCarts.forEach((cart) => {
      ...
      while (i < remainingPool.length && capacityLeft > 0) {
        ...
        const existing = cart.items.find(
          (it) => it.category === poolItem.category,
        );
        if (existing) {
          existing.platen += 1;
          existing.physicalPlates.push(poolItem.plate);
