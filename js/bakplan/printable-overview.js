...
<head>
    ...
    <style>
        ...
        * { box-sizing: border-box; }
        body { font-family: 'Poppins', Arial, sans-serif; margin: 0; padding: 4px; background: #fff; color: #000; font-size: 11px; line-height: 1.2; }
...
`;
  ...
  DAYS.forEach((day) => {
    const batches = generateBakplanSchedule(bakplanData, day, customCarts);
    ...
    if (batches.length === 0) {
      html += `<p style="padding: 6px; color: #666;">Geen producten om te bakken op ${day}.</p>`;
    } else {
      batches.forEach((batch) => {
                        ...
                        <table class="baktable">
                            <thead>
                                <tr class="print-repeating-header">
                                    <th colspan="4">${day.toUpperCase()} - BATCH ${batch.batchNumber}</th>
                                </tr>
                                <tr>
                                    <th>Kar</th>
                                    <th>Categorie</th>
        ...
        batch.carts.forEach((cart) => {
          ...
          cart.items.forEach((item, itemIdx) => {
            const plateRowsHtml = (item.physicalPlates || [])
              .map((p, pIdx) => {
                const prodsStr = p.products
                  .map((pr) => {
                    const stuks =
                      pr.pieces !== undefined
                        ? pr.pieces
                        : Math.round(pr.val * 12);
    ...
    if (batches.length > 0) {
          ...
          (c.items || []).forEach((it) => {
            (it.physicalPlates || []).forEach((pp) => {
              (pp.products || []).forEach((p) => {
                ...
                const stuks =
                  p.pieces !== undefined ? p.pieces : Math.round(p.val * 12);
                productStats[desc].total += stuks;
                productStats[desc].byBatch[b.batchNumber] =
                  (productStats[desc].byBatch[b.batchNumber] || 0) + stuks;
              });
            });
          });
        });
      ...
      const sortedProds = Object.keys(productStats).sort((a, b) =>
        a.localeCompare(b),
      );
      const batchChunkSize = 5;
      const totalBatchChunks = Math.max(
        1,
        Math.ceil(maxBatchNum / batchChunkSize),
      );
      ...
      for (let bChunkIdx = 0; bChunkIdx < totalBatchChunks; bChunkIdx++) {
                        ...
                        <tbody>
                            ${prods
                              .map((prodName) => {
                                ...
                                const batchCells = currentBatches
                                  .map((b) => {
        ...
        let batchLabel =
          totalBatchChunks > 1 ? ` (Batches ${startBatch}-${endBatch})` : "";

        html += `
                    <div class="summary-page">
                        <div class="day-header">${day.toUpperCase()} - PRODUCTEN OVERZICHT PER BATCH${batchLabel}</div>
