...
import {
  getRememberedPerPlaatMap,
  syncBakplanPerPlaatMemory,
} from "./utils.js";
    ...
    for (let row of rows) {
      ...
      for (let item of row.items) {
        if (mergedItems.length > 0) {
          ...
          if (
            item.x - last.x < 35 &&
            (item.text.startsWith("(") || item.text === "()")
          ) {
            last.text += item.text;
            continue;
          }
      ...
      const priceItem = mergedItems.find(
        (item) => item.x > ceItem.x && /^\d+[\.,]\d{2}$/.test(item.text),
      );
      ...
      const promoItem = mergedItems.find(
        (item) =>
          item.x > priceItem.x &&
          item.x < priceItem.x + 80 &&
      ...
      const numericTrailingItems = mergedItems
        .filter(
          (item) =>
      ...
      if (numericTrailingItems.length >= 2) {
        const dervingItem =
          numericTrailingItems[numericTrailingItems.length - 1];
        const opleggenItem =
          numericTrailingItems[numericTrailingItems.length - 2];
      ...
      const cleanPrice = priceCleanMatch
        ? parseFloat(priceCleanMatch[1].replace(",", "."))
        : null;
      ...
      let catObj = resultData.find(
        (c) => c.name.toLowerCase() === currentCatName.toLowerCase(),
      );
      if (!catObj) {
        catObj = {
          id:
          ...
          name: currentCatName,
          cartType: currentCatName.toLowerCase().includes("ontdooi")
            ? "ontdooi"
            : "normaal",
          collapsed: false,
          items: [],
      ...
      let prodObj = catObj.items.find(
        (p) => p.omschrijving.toLowerCase() === descVal.toLowerCase(),
      );
      if (!prodObj) {
        ...
        if (
          matchedExisting &&
          matchedExisting.perPlaat !== null &&
          matchedExisting.perPlaat !== undefined &&
          matchedExisting.perPlaat !== ""
        ) {
          initialPerPlaat = matchedExisting.perPlaat;
        } else if (
          rememberedMap[key] !== undefined &&
          rememberedMap[key] !== null
        ) {
        ...
        if (
          prodObj.perPlaat === null ||
          prodObj.perPlaat === undefined ||
          prodObj.perPlaat === ""
        ) {
          const key = descVal.trim().toLowerCase();
